...
Static Void fileproblem()
{
  ...
  printf("!\n");
  if (pcxtemp)
/* p2c: bm2font.pas, line 99: Note: Called procedure Exec [285] */
    Exec("rm", "pcxtemp.tmp");
  if (giftemp)
/* p2c: bm2font.pas, line 99: Note: Called procedure Exec [285] */
    Exec("rm", "giftemp.tmp");
 ...
 * Note: Can't interpret size in BLOCKREAD [174] */
  ...
  cutgrey = false;
  if (pcxtemp) {
/* p2c: bm2font.pas, line 127: Note: Called procedure Exec [285] */
    Exec("rm", "pcxtemp.tmp");
    pcxtemp = false;
  }
}
...
/* p2c: bm2font.pas, line 183:
 * Note: Can't interpret size in BLOCKREAD [174] */
      }
    }
  } while (checksum != 0);
/* p2c: bm2font.pas, line 155:
 * Note: Can't interpret size in BLOCKREAD [174] */
  if (bitmap != NULL)
    fclose(bitmap);
  bitmap = NULL;
  ...
  tmpfil = NULL;   /*21:*/
  if (giftemp) {   /*:21*/
/* p2c: bm2font.pas, line 193: Note: Called procedure Exec [285] */
    Exec("rm", "giftemp.tmp");
    giftemp = false;
    pcxtemp = true;
  } else {
...
/* p2c: bm2font.pas, line 243:
 * Note: Can't interpret size in BLOCKWRITE [174] */
    if (posbit > xratio) {
      FORLIM1 = mapinlen;
      for (gbufp = 0; gbufp < FORLIM1; gbufp++)
	b3[gbufp] = ((unsigned)(b3[gbufp] + b2[gbufp])) >> 1;
/* p2c: bm2font.pas, line 248:
 * Note: Can't interpret size in BLOCKWRITE [174] */
      _SETIO(fwrite(b3, mapinlen, 1, tmpfil) == 1, FileWriteError);
      filestat = P_ioresult;
      if (filestat != 0) {
...
struct LOC_gifint *LINK;
{
  ...
  if (gbufpp != LINK->hh)
    return;
  LINK->codepos = 0;
/* p2c: bm2font.pas, line 269:
 * Note: Can't interpret size in FILLCHAR [174] */
  memset((Anyptr)LINK->gifbuf, 0, (long)LINK->linebytes);
  FORLIM = LINK->hh;
  for (i = 0; i < FORLIM; i++) {
 ...
 * Note: Possible string truncation in assignment [145] */
  ...
  if (V.modus != 0) {
    memset((Anyptr)V.gifbuf, 0, (long)V.linebytes);
    for (i = 1; i <= vh; i++) {
...
/* p2c: bm2font.pas, line 342:
 * Note: Can't interpret size in BLOCKWRITE [174] */
    V.linepos = 0;
    V.logvr = 0;
    fseek(tmpfil, 0L, 0);
  }
/* p2c: bm2font.pas, line 341:
 * Note: Can't interpret size in FILLCHAR [174] */
  bpp = b << 5;
  bpp >>= 5;
  bpp++;
...
struct LOC_iffint *LINK;
{
  ...
  fread(b1, sizeof(uchar) * 2, 1, bitmap);
  LINK->bl = b1[0];
  for (k = 1; k <= 3; k++)
...
struct LOC_iffint *LINK;
{
  fread(b1, sizeof(uchar), 1, bitmap);
...
/* p2c: bm2font.pas, line 452:
 * Note: Can't interpret size in BLOCKREAD [174] */
    fread(&LINK->bps, 1, 1, bitmap);
/* p2c: bm2font.pas, line 452:
 * Note: Can't interpret size in BLOCKREAD [174] */
    fread(&LINK->b, 1, 1, bitmap);
/* p2c: bm2font.pas, line 453:
 * Note: Can't interpret size in BLOCKREAD [174] */
    fread(&LINK->b, 1, 1, bitmap);
    LINK->compr = LINK->b;
    for (i = 1; i <= 4; i++)
      fread(&xratio, 1, 1, bitmap);
/* p2c: bm2font.pas, line 454:
 * Note: Can't interpret size in BLOCKREAD [174] */
/* p2c: bm2font.pas, line 454:
 * Note: Can't interpret size in BLOCKREAD [174] */
 ...
 * Note: Can't interpret size in BLOCKREAD [174] */
  ...
  if (!strcmp(LINK->bmstr, "CRNG")) {
    ...
    LINK->bmoff = iffword(LINK);
    LINK->bmoff = iffword(LINK);
/* p2c: bm2font.pas, line 463:
 * Note: Can't interpret size in BLOCKREAD [174] */
    fread(&LINK->b, 1, 1, bitmap);
/* p2c: bm2font.pas, line 464:
 * Note: Can't interpret size in BLOCKREAD [174] */
...
/* p2c: bm2font.pas, line 483:
 * Note: Can't interpret size in BLOCKREAD [174] */
    }  /*:47*/
/* p2c: bm2font.pas, line 482:
 * Note: Can't interpret size in BLOCKREAD [174] */
    if (V.bps == 4) {   /*48:*/
      gi = 0;
      FORLIM1 = V.hh;
...
Static Void bmpint()
{
  ...
  for (i = 0; i <= col; i++) {
    ...
    hell[i] = (long)((gpal[i][0] * 0.287 + gpal[i][1] * 0.589 + gpal[i]
			[2] * 0.114) / (256.0 / (col + 1)));
  }
  if (bmsize > 12)
    bmoff -= (col + 1) * 4;
...
Char *argv[];
{
  ...
  greymem = (uchar *)Malloc(sizeof(pxla));   /*104:*/
  strcpy(cref, "!()+,-./0123456789:;<=>?");
  strcat(cref, "ABCDEFGHIJKLMNOPQRSTUVWXYZ");
/* p2c: bm2font.pas, line 855:
 * Note: Possible string truncation in assignment [145] */
  strcat(cref, "abcdefghijklmnopqrstuvwxyz");
/* p2c: bm2font.pas, line 856:
 * Note: Possible string truncation in assignment [145] */
  ...
  newwidth = 0;
  newheight = 0;
  printf("This is BitMapTOfont, version 2.0 of january 93, via p2c\n");
 ...
 * Note: Line breaker spent 14.0 seconds, 5000 tries on line 3970 [251] */
	}
...
/* p2c: bm2font.pas, line 1107:
 * Note: Can't interpret size in BLOCKWRITE [174] */
/* p2c: bm2font.pas, line 1110: Note: Can't interpret size in MOVE [174] */
	  memmove((Anyptr)(&greymem[pgreymem]), (Anyptr)b3, (long)gbufpp);
	  pgreymem += gbufpp;
	}
/* p2c: bm2font.pas, line 1111:
 * Note: Can't interpret size in FILLCHAR [174] */
...
/* p2c: bm2font.pas, line 1114:
 * Note: Can't interpret size in BLOCKWRITE [174] */
/* p2c: bm2font.pas, line 1117: Note: Can't interpret size in MOVE [174] */
	    memmove((Anyptr)(&greymem[pgreymem]), (Anyptr)b3, (long)gbufpp);
	    pgreymem += gbufpp;
	  }  /*:100*/
	}
 ...
 * Note: Can't interpret size in BLOCKREAD [174] */
  ...
  while (mapheight > 0) {   /*:118*/
	...
	pheight[k][c] = mapheight;
      dsize = floor(vres / truehres * 72.27 * 65536L * 16 + 0.5);
      tabdsize[k] = dsize;
      c++;
      if (c == mincharpf || (mapheight <= vres && maxx <= hres)) {
	FORLIM1 = mincharpf;
	for (c = 0; c < FORLIM1; c++) {
	  height[k][c] = floor(pheight[k][c] / truevres * 4736286.7227 /
				     dsize * 16777215L + 0.5);
	  width[k][c] = floor(pwidth[k][c] / truehres * 4736286.7227 / dsize *
			      16777215L + 0.5);
	}
	k++;
	c = 0;
  ...
  /*106:*/
  if (pcxtemp)
/* p2c: bm2font.pas, line 1249: Note: Called procedure Exec [285] */
    Exec("rm", "pcxtemp.tmp");
  if (giftemp)
/* p2c: bm2font.pas, line 1249: Note: Called procedure Exec [285] */
    Exec("rm", "giftemp.tmp");
